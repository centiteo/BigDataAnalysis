...
import com.intel.bigdata.analysis.index.Index;
...
import com.intel.bigdata.analysis.index.util.ByteArrayUtils;
import com.intel.bigdata.analysis.index.util.RegionUtil;
import com.intel.bigdata.analysis.query.Query;
import com.intel.bigdata.analysis.query.QueryResult;
import com.intel.bigdata.analysis.query.ResultComparator;
import com.intel.bigdata.analysis.query.SearchMode;

/**
 * This strategy search records via indexes. It search index entries directly by
 * a {@link com.intel.its.server.search.IndexRangeSet} which resolved from
 * {@link com.intel.its.server.search.SearchStrategyDecider}, get target record
 * row key from index entry, then get this record by row key. index entry and
 * target record always reside at the same region, so retrieving record by index
 * is very fast. NOTE: This strategy is generic, it does NOT depend on any table
 * data structure!!
 */
public class IndexBasedSearchStrategy implements SearchStrategy {
   ...
   * {@link com.intel.its.server.search.IndexBasedSearchStrategy} so, this
   * method build row key using fields from left to right none-null qualifier.
   */
