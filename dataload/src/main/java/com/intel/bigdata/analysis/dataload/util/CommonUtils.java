package com.intel.bigdata.analysis.dataload.util;
...
import com.intel.bigdata.analysis.dataload.transform.HBaseTableSpec;
import com.intel.bigdata.analysis.dataload.transform.HBaseTableSpec.ColumnSpec;
...
import com.intel.bigdata.analysis.query.RecordServiceEndpoint;
...
public class CommonUtils {
  ...
  private static String idpVersion = new String();

  public static String getIdpVersion() {
    return idpVersion;
  }

  public static void setIdpVersion(String idpVersion) {
    CommonUtils.idpVersion = idpVersion;
  }
  ...
  public static void createTable(Configuration conf, HBaseAdmin hbaseAdmin,
    ...
    try {
      ...
      htd.addCoprocessor(RecordServiceEndpoint.class.getName(),
          new Path(conf.get(Constants.HBASE_COPROCESSOR_LOCATION)), 1001, null);
      if (LOGGER.isDebugEnabled()) {
  ...
  public static String getTempDir(JobClient client) {
    ...
    // for IDP 2.5.1 verion, if IDP 3.X version, interface changes.
  ...
  public static ArrayList<String> generateSplitKey(int regionQuantity) {
    ...
    for (int i = 0; i < splitKeys.length; i++) {
      regionStartKeys.add(CommonUtils.convertByteArrayToString(splitKeys[i]));
    }
