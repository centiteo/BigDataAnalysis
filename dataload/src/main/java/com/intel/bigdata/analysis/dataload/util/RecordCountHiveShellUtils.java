...
public class RecordCountHiveShellUtils {
  ...
  public static void generateRecordCountHiveShell(Configuration conf)
    ...
    try {
      ...
      out.write("add jar /usr/lib/hive/lib/hive-contrib.jar;");
      out.newLine();
      ...
      if (CommonUtils.getIdpVersion().startsWith("3.")) {
        // if IDP 3.X version, interface changes.
        out.write("ROW FORMAT DELIMITED FIELDS TERMINATED BY \""
            + fieldDelimiter + "\"");
      } else {
  ...
  public static void generateTotalHiveShell(Configuration conf) {
    ...
    try {
      ...
      BufferedWriter output = new BufferedWriter(new FileWriter(
          generatedTotalHiveShellFile.getCanonicalPath()));
      // TODO: need to use upper hive version
      output.write("add jar /usr/lib/hive/lib/hive-contrib.jar;");
      output.newLine();
  ...
  public static String calculateRecordNumPerRegion(Configuration conf) {
    ...
    try {
      ...
      for (String rsHostName : rsList) {
        regionNumByRS = getCoreNumPerRS(idpHBaseMasterIpaddress, rsHostName) * 4;
        // calculate the max region number according to memory size and hbase
        // flush size(64MB)
        regionNumByGlobalFlush = (long) (getMemNumPerRS(
            idpHBaseMasterIpaddress, rsHostName) * 0.5 * 0.35 / (64 * 1024));
  ...
  public static long getCoreNumPerRS(String idpHBaseMasterIpaddress,
    ...
    try {
      prs = Runtime.getRuntime().exec(
          new String[] {
              "/bin/sh",
              "-c",
              "ssh " + idpHBaseMasterIpaddress + " ssh " + RegionServerHost
  ...
  public static long getMemNumPerRS(String idpHBaseMasterIpaddress,
      String RegionServerHost) {
    Process prs;
    long MemNum = 0;
    try {
