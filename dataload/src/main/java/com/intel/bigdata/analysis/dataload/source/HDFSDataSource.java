...
import com.intel.bigdata.analysis.dataload.io.HDFSFileObject;
...
public class HDFSDataSource extends DataSource {
  ...
  private List<FileObject> getFileList(Path hdfsDirDesc) throws IOException {
    ...
    if (fs.exists(hdfsDirDesc)) {
      ...
      if (fs.isFile(hdfsDirDesc)) {
        retList.add(new HDFSFileObject(serverAddr, hdfsDirDesc
            .getParent().toString(), hdfsDirDesc.getName()));
      } else {
        ...
        for (FileStatus file : files) {
          if (!file.isDir()) {
            ...
            retList.add(new HDFSFileObject(serverAddr, path
                .getParent().toString(), path.getName()));
          }else{
  ...
  public void closeFile(FileObject file, InputStream is,
    ...
    /*
    Path temp = new Path(file.getPath() + "/tmp/");
    if (!fs.exists(temp)) {
      fs.mkdirs(temp);
    }
    fs.rename(new Path(file.getCanonicalPath()), new Path(file.getPath()
        + "/tmp/" + file.getName()));
    */
  }
